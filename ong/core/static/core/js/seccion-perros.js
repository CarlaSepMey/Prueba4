...
$(document).ready(function() {
    ...
    var url_razas = 'https://api.thedogapi.com/v1/breeds';
    ...
    $('#tipoRaza').change(function(){ 
        ...
        if(value_id != 9999){
            var url_imgen = 'https://api.TheDogAPI.com/v1/images/search?';
    
            $.get(url_imgen, { option: value_text }, function(response) { 
                $('#imagenPerro').append('<img src="'+response[0].url+'" alt="Girl in a jacket" width="250" height="300">');
            });
        }else{
            $('#imagenPerro').append('<p>Seleccione una raza para mostrar</p>');
        }
